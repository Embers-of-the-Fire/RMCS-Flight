...
private:
    ...
    void on_odom_frame(const capture_Image_List_t* stream) {
        ...
        if (data_len == sizeof(ros_odom_convert_complete_t)) {
            ...
            // pose covariance diagonal (cov[0]=xx, cov[4]=yy, cov[8]=zz) encoded ×1e9
            snap.cov_xx = static_cast<float>(d->cov[0]) / 1e9f;
            snap.cov_yy = static_cast<float>(d->cov[4]) / 1e9f;
            snap.cov_zz = static_cast<float>(d->cov[8]) / 1e9f;
